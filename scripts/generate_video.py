#!/usr/bin/env python3
"""Prompt -> optional Codex or Pi/Blender physics draft -> fal Seedance 2.0 -> MP4.
...
def pi_command(pi: str, extension: Path | None = None) -> list[str]:
...
def codex_command(codex: str, workspace: Path, audit_path: Path, skill_override: str) -> list[str]:
    command = [
        codex,
        "exec",
        "--ignore-user-config",
...
4. Write edit_prompt.txt: a concise English instruction for Seedance 2.0 to restyle
   @Video1 (this proxy) into the requested photorealistic scene. Preserve the proxy's
   object count, geometry, timing, camera, trajectories, contact order, phase changes,
   and causal sequence. Restore appropriate colors, materials, detail and lighting.
   Describe the intended scene and critical physical constraints explicitly. No
   command-line flags, no Markdown, no request to invent new motion or cut scenes.
...
def validate_draft(workspace: Path, frame_count: int) -> str:
    ...
    if "@Video1" not in edit_prompt:
        raise RuntimeError("edit_prompt.txt must explicitly reference @Video1.")
    return edit_prompt


def create_draft(
    prompt: str, args: argparse.Namespace, run_dir: Path, agent: str, blender: str, ffmpeg: str, ffprobe: str
) -> tuple[Path, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="videococo-draft-") as temporary:
        ...
        if args.agent == "pi":
            ...
            command = pi_command(agent, extension)
            env = pi_environment(runtime)
        else:
            skill_override = isolate_skill_context(agent, workspace, runtime)
            command = codex_command(agent, workspace, workspace / "audit.json", skill_override)
            env = codex_environment(runtime)
        ...
        try:
            ...
            edit_prompt = validate_draft(workspace, args.duration * 24)
        finally:
            if (runtime / "auth.json").is_file():
    ...
    metadata = validate_proxy(run_dir / "proxy.mp4", ffprobe)
...
def validate_proxy(path: Path, ffprobe: str) -> dict[str, Any]:
    ...
    if not 0 < path.stat().st_size < 50_000_000:
    ...
    if not 640 * 640 <= pixels <= 834 * 1112:
        raise RuntimeError("Reference resolution is outside fal's supported pixel area. Resize it to 720p first.")
    return metadata


def reference_prompt(prompt: str) -> str:
    return (
...
def arguments_for(prompt: str, args: argparse.Namespace, video_url: str | None = None) -> dict[str, Any]:
    payload = {
        "prompt": prompt,
...
def collect_result(client: SyncClient, module: ModuleType, record_path: Path, output: Path, timeout: int) -> None:
    ...
    if endpoint not in (TEXT_ENDPOINT, REFERENCE_ENDPOINT):
        raise RuntimeError("Saved request is not a supported Seedance 2.0 endpoint.")
...
def parser() -> argparse.ArgumentParser:
    ...
    prompt.add_argument("--prompt", help="Scene description; the only required creative input.")
    prompt.add_argument("--prompt-file", type=Path, help="Read a UTF-8 scene description or edit instruction.")
    mode = result.add_mutually_exclusive_group()
    mode.add_argument("--direct", action="store_true", help="Text-to-video; skip the agent and Blender.")
    ...
    result.add_argument("--resolution", choices=("480p", "720p", "1080p"), default="720p")
    result.add_argument("--aspect-ratio", choices=tuple(PROXY_SIZES), default="16:9")
    result.add_argument("--audio", action=argparse.BooleanOptionalAction, default=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not prompt or not prompt.strip():
        cli.error("provide a nonempty --prompt or --prompt-file")
    prompt = prompt.strip()
    ...
    endpoint = TEXT_ENDPOINT if args.direct else REFERENCE_ENDPOINT
    if args.dry_run:
        print(
            json.dumps(
                {
                    ...
                    "input": arguments_for(prompt, args, "<uploaded proxy URL>" if not args.direct else None),
                    "agent": None
    ...
    if not args.direct and not proxy:
        proxy, prompt = create_draft(prompt, args, run_dir, agent, blender, ffmpeg, ffprobe)
    elif proxy:
    ...
    payload = arguments_for(prompt, args, video_url)
